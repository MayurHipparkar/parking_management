    ...
    <div class=" col-sm-9 container ">
        <div class="card shadow-lg">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0">Update Guard Form</h4>
            </div>
            <div class="card-body">
                <form action="${pageContext.request.contextPath}/update-guard" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control"
                                  ...
                                  rows="1"
                                  maxlength="255"
                                  required>${guardData.address}</textarea>
                    </div>
                    
                  <!--   hidden value id,and role -->
                     <input type="hidden" name="id" value="${guardData.id}" required>
                    <input type="hidden" name="role" value="${guardData.role}" required>

                    <!-- Status -->
                    <div class="mb-3">
                        <label for="status" class="form-label">Status</label>
                        <select class="form-select" id="status" name="status" required>
                            <option value="Active" ${guardData.status == 'Active' ? 'selected' : ''}>Active</option>
                            <option value="Inactive" ${guardData.status == 'Inactive' ? 'selected' : ''}>Inactive</option>
                        </select>
                    </div>

                    <!-- Buttons -->
                    <div class="text-end">
                        <button type="submit" class="btn btn-success">Update Guard</button>
